...
CANDIDATES = {
    "DENSE_MATCHED": {
        ...
        "q_rank": None,
        "k_rank": None,
        "expected_params": CANONICAL_PARAMS,
    },
}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="PB1-C matched dense Q/K control with 32-token truncated BPTT"
    )
...
def factorized_memory_feedback_archive_layer_fwd_stateful(
    ...
    """
    PB1-C stateful MemoryFeedbackArchive layer forward.

    Supports the two parameter layouts used by the research trainers:
      * factorized Q/K: m_wq_A @ (m_wq_B @ e), m_wk_A @ (m_wk_B @ e)
      * dense control: m_wq @ e, m_wk @ e

    The DENSE_MATCHED control therefore changes only the Q/K representation;
    all recurrent memory, routing, auxiliary supervision, and 32-token state
    handling remain identical to PB1-C.
    """
    if "m_wk" in layer and "m_wq" in layer:
        r = layer["m_wk"].shape[0]
        dense_qk = True
    elif "m_wk_A" in layer and "m_wq_A" in layer:
        r = layer["m_wk_A"].shape[0]
        dense_qk = False
    else:
        raise KeyError(
            "Layer must contain either dense m_wq/m_wk or factorized "
            "m_wq_A/m_wq_B/m_wk_A/m_wk_B parameters."
        )
    ...
    def step(carry, e_raw):
        ...
        H_current = (
            retain * H_current
            + (eta * write) * jnp.outer(val - old_current, k)
        )
        ...
        current_context = layer_norm(
            H_current @ q, layer["m_ln_g"], layer["m_ln_b"]
        )
        archive_context = layer_norm(
            H_archive @ q, layer["m_ln_g"], layer["m_ln_b"]
        )

        archive_mix = jax.nn.sigmoid(
            layer["m_w_archive_mix"] @ e + layer["m_b_archive_mix"]
        )
        ...
        proposal = (
            layer["m_proj_w"] @ jnp.concatenate([e_raw, context])
            + layer["m_proj_b"]
        )
        ...
        mamba_out = gate_s * (
            layer["s_proj_w"] @ y_s + layer["s_proj_b"]
        )
    ...
    if initial_state is None:
        initial_state = (
            jnp.zeros((r, r), dtype=jnp.float32),
            jnp.zeros((r, r), dtype=jnp.float32),
            jnp.zeros(layer["s_wu"].shape[0], dtype=jnp.float32),
        )

    final_state, outputs = jax.lax.scan(step, initial_state, x_seq)
...
def factorized_deep_supervision_fwd_stateful(
    ...
    if recurrent_states is None:
        if "m_wk" in p["layers"]:
            layer_count = len(p["layers"]["m_wk"])
            state_rank = lambda i: p["layers"]["m_wk"][i].shape[0]
        else:
            layer_count = len(p["layers"]["m_wk_A"])
            state_rank = lambda i: p["layers"]["m_wk_A"][i].shape[0]

        recurrent_states = tuple(
            (
                jnp.zeros((state_rank(i), state_rank(i)), dtype=jnp.float32),
                jnp.zeros((state_rank(i), state_rank(i)), dtype=jnp.float32),
...
def main() -> None:
    ...
    print("=" * 80)
    print("MODUS_X PB1-C — MATCHED DENSE Q/K CONTROL")
    print("=" * 80)
    ...
    print("Q/K computation              : DENSE m_wq @ e / m_wk @ e")
    ...
    if "m_wq" not in dense_params["layers"] or "m_wk" not in dense_params["layers"]:
        raise RuntimeError(
            "Dense matched control requires native dense m_wq/m_wk parameters."
        )
    if "m_wq_A" in dense_params["layers"] or "m_wk_A" in dense_params["layers"]:
        raise RuntimeError(
            "Dense matched control must not contain factorized Q/K parameters."
        )

    measured_count = dense_count
    expected_count = candidate["expected_params"]
    factor_stats = {"qk_savings": 0}

    print("=" * 80)
    print("DENSE MATCHED CONTROL INITIALIZATION")
    print("=" * 80)
    print(f"Dense control parameters     : {dense_count:,}")
    print(f"Canonical expected parameters: {expected_count:,}")
    print(f"Budget error                 : {measured_count - CANONICAL_PARAMS:+,}")
    print("Q/K representation           : DENSE m_wq/m_wk")

    if measured_count != expected_count:
        raise RuntimeError(
            f"Dense control count mismatch: expected {expected_count:,}, "
            f"found {measured_count:,}."
        )

    print("Dense parameter count        : PASS")

    # ------------------------------------------------------------------------
    # DENSE MATCHED FORWARD
    # ------------------------------------------------------------------------
    ...
    # Smoke-test the exact matched dense training forward before placing on TPU.
    ...
    if not isinstance(smoke_outputs, tuple) or len(smoke_outputs) != 4:
        raise RuntimeError(
            "Dense matched training forward did not return four outputs."
        )

    for output in smoke_outputs:
        if not np.all(np.isfinite(np.asarray(output))):
            raise RuntimeError(
                "Non-finite value found in dense matched smoke forward."
            )

    print("Dense matched forward        : PASS")
    ...
    # Matched dense control keeps Q/K as native dense m_wq/m_wk tensors.
    # Recurrent state width is therefore the dense key dimension (512).
    if "m_wk" in factorized_params["layers"]:
        state_layers = factorized_params["layers"]["m_wk"]
    elif "m_wk_A" in factorized_params["layers"]:
        state_layers = factorized_params["layers"]["m_wk_A"]
    else:
        raise KeyError("Expected dense m_wk or factorized m_wk_A parameters.")

    recurrent_states = tuple(
        (
            jnp.zeros((state_layers[i].shape[0], state_layers[i].shape[0]), dtype=jnp.float32),
            jnp.zeros((state_layers[i].shape[0], state_layers[i].shape[0]), dtype=jnp.float32),
    ...
    config = {
        "experiment": "PB1-C matched dense control",
        "model": MODEL_NAME,
