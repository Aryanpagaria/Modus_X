...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Modus_X final C1 low-rank Q/K trainer with 32-token truncated BPTT"
    )
...
def build_config() -> ModelConfig:
    return ModelConfig(
        ...
        hidden_dim=FINAL_C1["hidden_dim"],
        ax_res=FINAL_C1["ax_res"],
        n_layers=CANONICAL["n_layers"],
        n_heads_attn=CANONICAL["n_heads"],
        seq_len=CANONICAL["seq_len"],
        mamba_state_dim=FINAL_C1["mamba_state_dim"],
...
def main() -> None:
    ...
    print("=" * 80)
    print("MODUS_X — FINAL C1 TRAINER")
    print("=" * 80)
    print(f"Project root                 : {PROJECT_ROOT}")
    print(f"Language dir                 : {LANGUAGE_DIR}")
    print(f"Architecture                 : {final_arch['name']}")
    print(f"Hidden dimension             : {final_arch['hidden_dim']}")
    print(f"Mamba state dimension        : {final_arch['mamba_state_dim']}")
    print(f"ax_res                       : {final_arch['ax_res']}")
    print(f"Q rank                       : {final_arch['q_rank']}")
    print(f"K rank                       : {final_arch['k_rank']}")
    print(f"Expected parameters          : {final_arch['expected_params']:,}")
    ...
    print("Q/K computation              : TRUE factorized A @ (B @ e)")
    print("Training protocol            : 32-token truncated BPTT; continuous forward state")
    print("Architecture search          : FROZEN — C1 only")
    print()

    if jax.default_backend() != "tpu":
        raise RuntimeError(
            f"Expected TPU backend, found {jax.default_backend()}. "
            "The final trainer requires TPU execution."
        )

    if args.batch != 1:
        raise ValueError(
            "The final C1 protocol requires --batch 1 because recurrent state "
            "is carried between optimizer updates."
        )
    ...
    if args.input_seq_len != CANONICAL["seq_len"]:
        raise ValueError("The final C1 trainer requires sequence length 512.")

    if args.loss_tail != args.input_seq_len:
        raise ValueError("The final C1 trainer requires loss_tail == input sequence length.")
    ...
    measured_count = int(count_params(factorized_params))
    expected_count = final_arch["expected_params"]

    print(f"Final C1 parameters           : {measured_count:,}")
    print(f"Expected C1 parameters       : {expected_count:,}")
    ...
    if measured_count != expected_count:
        raise RuntimeError(
            "FINAL C1 PARAMETER COUNT MISMATCH: "
            f"expected {expected_count:,}, found {measured_count:,}. "
            "STOP — do not train until the parameter tree is reconciled."
        )
    ...
    for name in required_factorized:
        if name not in layers:
            raise RuntimeError(f"Missing required factorized parameter family: {name}")
    for name in forbidden_dense:
        if name in layers:
    ...
    smoke_rng = np.random.default_rng(10_000 + args.seed)
    smoke_x = jnp.asarray(
        smoke_rng.integers(
            0,
    ...
    smoke_outputs = fwd_fn(factorized_params, smoke_x)

    if not isinstance(smoke_outputs, tuple) or len(smoke_outputs) != 4:
        raise RuntimeError("Final C1 forward did not return four outputs.")

    for output in smoke_outputs:
        if not np.all(np.isfinite(np.asarray(output))):
            raise RuntimeError("Non-finite value found in final C1 smoke forward.")
    ...
    stop_steps = (
        ...
        else min(total_steps, math.ceil(args.stop_chars / chars_per_step))
    )

    if stop_steps <= 0:
        raise ValueError("--stop-chars must produce at least one step.")

    checkpoint_steps = max(
        1,
        round(args.checkpoint_chars / chars_per_step),
    )
    ...
    opt_state = tx.init(factorized_params)
    ...
    factorized_params = jax.device_put(factorized_params, replicated)
    opt_state = jax.device_put(opt_state, replicated)

    recurrent_states = tuple(
        (
            jnp.zeros(
                (factorized_params["layers"]["m_wk_A"][i].shape[0],
                 factorized_params["layers"]["m_wk_A"][i].shape[0]),
                dtype=jnp.float32,
            ),
            jnp.zeros(
            ...
            jnp.zeros(
                factorized_params["layers"]["s_wu"][i].shape[0],
                dtype=jnp.float32,
            ),
        )
    ...
    config = {
        "experiment": "PB1-C final C1 low-rank Q/K training",
        "model": MODEL_NAME,
        "architecture": final_arch["name"],
        "architecture_frozen": True,
        "hidden_dim": final_arch["hidden_dim"],
        "mamba_state_dim": final_arch["mamba_state_dim"],
        "ax_res": final_arch["ax_res"],
        "q_rank": final_arch["q_rank"],
        "k_rank": final_arch["k_rank"],
        ...
        "auxiliary_layers": list(CANONICAL["aux_layers"]),
        "future_target_count": CANONICAL["future_target_count"],
        "seed": args.seed,
        "devices": [str(d) for d in jax.devices()],
        "true_low_rank_matmuls": True,
        "factorized_qk_shapes": {
            "m_wq_A": list(expected_shapes["m_wq_A"]),
            "m_wq_B": list(expected_shapes["m_wq_B"]),
            "m_wk_A": list(expected_shapes["m_wk_A"]),
            "m_wk_B": list(expected_shapes["m_wk_B"]),
        },
        "recurrent_state_shapes": {
            "H_current": [final_arch["q_rank"], final_arch["k_rank"]],
            "H_archive": [final_arch["q_rank"], final_arch["k_rank"]],
        },
        "recurrent_chunk": 32,
        "gradient_horizon": 32,
    ...
    for step in range(start_step + 1, stop_steps + 1):
        ...
        if len(chunk) != args.train_chunk_len + 1:
            raise RuntimeError(
                f"Training stream exhausted at step {step:,}: "
        ...
        if step % checkpoint_steps == 0 or step == stop_steps:
            block_tree((factorized_params, opt_state))
    ...
    if stop_steps == total_steps:
        ...
        final = {
            "architecture": final_arch["name"],
            "processed_characters": stop_steps * chars_per_step,
            "validation_bpc": rows[-1]["val_bpc"] if rows else None,
            "best_validation_bpc": min((row["val_bpc"] for row in rows), default=None),
            "test_bpc": test_bpc,
        ...
        atomic_json(outdir / "final_result.json", final)

        print("=" * 80)
        print("FINAL C1 RESULT")
        print("=" * 80)
        print(f"Architecture                : {final_arch['name']}")
        print(f"Parameters                  : {measured_count:,}")
        print(f"Budget error                : {measured_count-CANONICAL_PARAMS:+,}")
        print(f"Validation BPC              : {final['validation_bpc']:.6f}")
        print(f"Best validation BPC         : {final['best_validation_bpc']:.6f}")
        print(f"Test BPC                    : {test_bpc:.6f}")
        print("Training status             : COMPLETE")
    else:
        print("=" * 80)
        print("FINAL C1 SCREEN STOPPED")
        print("=" * 80)
        print(f"Stopped after {stop_steps:,} steps.")
        print("Checkpoint retained for resume.")
