...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="PB1-C actual low-rank Q/K training screen"
    )
...
def factorized_deep_supervision_fwd(
    p: Dict[str, Any],
    x_ids: jax.Array,
    cfg: ModelConfig,
...
def main() -> None:
    ...
    print("=" * 80)
    print("MODUS_X PB1-C — ACTUAL LOW-RANK Q/K TRAINING SCREEN")
    print("=" * 80)
    ...
    fwd_fn = lambda p, x: factorized_deep_supervision_fwd(
        p,
        x,
        cfg,
    ...
    config = {
        "experiment": "PB1-C actual low-rank Q/K training screen",
        "model": MODEL_NAME,
    ...
    for step in range(
        ...
        factorized_params, opt_state, loss = update(
            factorized_params,
            opt_state,
            x,
            y,
